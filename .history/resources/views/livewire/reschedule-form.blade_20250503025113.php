...
<div class="max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-10 bg-white border my-2  shadow-md">
  ...
  <div class="grid grid-cols-2 md:grid-cols-3 gap-8 md:gap-12">
    ...
    <div class="text-center">
            ...
            <h3 class="text-gray-500">Randevu details</h3>
    ...
    @endif
    <div>
        <h2 class="text-xl font-bold mb-2">Uygun Bir saat Seçin</h2>
        <div class="flex flex-wrap">
            @foreach ($timeSlots as $slot)
                <button class="m-2 p-2 bg-blue-500 text-white rounded hover:bg-blue-700"
                type="button"
                wire:click="updateAppointment('{{$slot}}')"
                wire:confirm="Gerçekten randevuyu güncellemek istiyor musunuz? {{$appointment_details->appointment_date .', '. date('H:i A',strtotime($appointment_details->appointment_time))}} to {{ $selectedDate }}, {{ $slot }} ?">
