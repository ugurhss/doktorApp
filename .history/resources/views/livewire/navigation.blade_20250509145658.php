    ...
    <nav class="relative max-w-[85rem] w-full mx-auto md:flex md:items-center md:justify-between md:gap-3 py-2 px-4 sm:px-6 lg:px-8">
      ...
      <div id="hs-navbar-hcail" class="hs-collapse hidden overflow-hidden transition-all duration-300 basis-full grow lg:block lg:w-auto lg:basis-auto lg:order-2 lg:col-span-6" aria-labelledby="hs-navbar-hcail-collapse">
        <div class="flex flex-col gap-y-4 gap-x-0 mt-5 lg:flex-row lg:justify-center lg:items-center lg:gap-y-0 lg:gap-x-7 lg:mt-0">
          ...
          <div>
            <a class="inline-block text-black hover:text-gray-600 focus:outline-none" href="{{ route('news.byMenu', ['menu' => 'umut-dolu-videolar-5']) }}">Umut Dolu Videolar</a>
          </div>
          <div>
            <a class="inline-block text-black hover:text-gray-600 focus:outline-none" href="{{ route('news.byMenu', ['menu' => 'faliyetlerimiz-6']) }}">Faaliyetlerimiz</a>
          </div>
          <div>
            <a class="inline-block text-black hover:text-gray-600 focus:outline-none" href="{{ route('news.byMenu', ['menu' => 'medya-haberlerimiz-7']) }}">Medya Haberleri</a>
          </div>
          <div>
            <a class="inline-block text-black hover:text-gray-600 focus:outline-none" href="{{ route('news.byMenu', ['menu' => 'nasıl-destek-olabilirim-8']) }}">Nasıl Destek Olunur?</a>
          </div>
          <div>
            <a class="inline-block text-black hover:text-gray-600 focus:outline-none" href="{{ route('news.byMenu', ['menu' => 'iletisim-9']) }}">İletişim</a>
          </div>
        </div>
      </div>
    </nav>
