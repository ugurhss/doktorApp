...
<header class="flex flex-wrap md:flex-col md:justify-start md:flex-nowrap z-50 w-full bg-white border-b border-gray-200">
    <div class="max-w-[85rem] mx-auto w-full px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between gap-x-3 pt-2">
        ...
        <!-- Sol tarafta logo veya marka adı -->
        <a class="font-semibold text-xl text-black focus:outline-none" href="#">Logo</a>
        ...
        <div class="flex items-center gap-x-1 lg:gap-x-2 ms-auto py-1 lg:ps-6 lg:order-3 lg:col-span-3">
          ...
          <div class="lg:hidden">
            <button type="button" class="hs-collapse-toggle size-9.5 flex justify-center items-center text-sm font-semibold rounded-xl border border-gray-200 text-black hover:bg-gray-100 focus:outline-hidden focus:bg-gray-100 disabled:opacity-50 disabled:pointer-events-none" id="hs-navbar-hcail-collapse" aria-expanded="false" aria-controls="hs-navbar-hcail" data-hs-collapse="#hs-navbar-hcail">
              ...
              <svg class="hs-collapse-open:block hidden shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
            </button>
          </div>
        </div>
      </div>
    ...
    <nav class="relative max-w-[85rem] w-full mx-auto md:flex md:items-center md:justify-between md:gap-3 py-2 px-4 sm:px-6 lg:px-8">
      ...
      <div id="hs-navbar-hcail" class="hs-collapse hidden overflow-hidden transition-all duration-300 basis-full grow lg:block lg:w-auto lg:basis-auto lg:order-2 lg:col-span-6" aria-labelledby="hs-navbar-hcail-collapse">
        <div class="flex flex-col gap-y-4 gap-x-0 mt-5 lg:flex-row lg:justify-center lg:items-center lg:gap-y-0 lg:gap-x-7 lg:mt-0">
          ...
          <!-- Hizmetlerimiz -->
          <div>
            <a class="relative inline-block text-black focus:outline-hidden before:absolute before:bottom-0.5 before:start-0 before:-z-1 before:w-full before:h-1 before:bg-lime-400" href="#" aria-current="page">Hizmetlerimiz</a>
          </div>

          <!-- Diğer Menüler -->
          <div>
            <a class="inline-block text-black hover:text-gray-600 focus:outline-hidden focus:text-gray-600"
               href="{{ route('news.byMenu', ['menu' => 'hakkımızda-1']) }}">Hakkımızda</a>
          </div>
          <div>
            <a class="inline-block text-black hover:text-gray-600 focus:outline-hidden focus:text-gray-600"
               href="{{ route('news.byMenu', ['menu' => 'Projeler-2']) }}">Projeler</a>
          </div>
          <div>
            <a class="inline-block text-black hover:text-gray-600 focus:outline-hidden focus:text-gray-600"
               href="{{ route('news.byMenu', ['menu' => 'kanser-hakkında-3']) }}">Kanser Hakkında</a>
          </div>
          <div>
            <a class="inline-block text-black hover:text-gray-600 focus:outline-hidden focus:text-gray-600"
               href="{{ route('news.byMenu', ['menu' => 'haberler-4']) }}">Haberler</a>
          </div>
          <div>
            <a class="inline-block text-black hover:text-gray-600 focus:outline-hidden focus:text-gray-600"
               href="{{ route('video.byMenu', ['menu' => 'umut-dolu-videolar-5']) }}">Umut Dolu Videolar</a>
          </div>
          <div>
            <a class="inline-block text-black hover:text-gray-600 focus:outline-hidden focus:text-gray-600"
               href="{{ route('video.byMenu', ['menu' => 'faliyetlerimiz-6']) }}">Faaliyetlerimiz</a>
          </div>
          <div>
            <a class="inline-block text-black hover:text-gray-600 focus:outline-hidden focus:text-gray-600"
               href="{{ route('video.byMenu', ['menu' => 'medya-haberlerimiz-7']) }}">Medya Haberleri</a>
          </div>
          <div>
            <a class="inline-block text-black hover:text-gray-600 focus:outline-hidden focus:text-gray-600"
               href="{{ route('video.byMenu', ['menu' => 'nasıl-destek-olabilirim-8']) }}">Nasıl Destek Olunur?</a>
          </div>
          <div>
            <a class="inline-block text-black hover:text-gray-600 focus:outline-hidden focus:text-gray-600"
               href="{{ route('video.byMenu', ['menu' => 'iletisim-9']) }}">İletişim</a>
          </div>
        </div>
      </div>
    </nav>
