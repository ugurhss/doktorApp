                ...
                <div class="aspect-w-16 aspect-h-9 mb-4">
                    @php
                        ...
                        if (Str::contains($link, 'youtube.com/watch?v=')) {
                            ...
                            // Handle additional parameters in YouTube URL
                            $youtubeId = Str::before($youtubeId, '&');
                        } elseif (Str::contains($link, 'youtu.be/')) {
                            $youtubeId = Str::afterLast($link, '/');
                            // Handle additional parameters in YouTube short URL
                            $youtubeId = Str::before($youtubeId, '?');
                        }
                    @endphp
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        // Initialize local video players
        const players = Array.from(document.querySelectorAll('.js-plyr')).map(
            player => new Plyr(player)
        );

        // Initialize YouTube players
        document.querySelectorAll('.youtube-player').forEach(playerEl => {
            ...
            // Create the iframe manually if needed
            playerEl.innerHTML = `
                <iframe
                    src="https://www.youtube.com/embed/${videoId}?origin=${window.location.origin}&enablejsapi=1"
                    allowfullscreen
                    allowtransparency
                    allow="autoplay"
                    frameborder="0"
