<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        {{-- <a href="{{ route('news.index') }}" class="ml-3 text-sm font-medium text-gray-700 hover:text-indigo-600 dark:text-gray-400 dark:hover:text-white">Haberler</a> --}}
                    </div>
                </li>
                <li aria-current="page">
                    <div class="flex items-center">
                        <i class="fas fa-chevron-right text-gray-400 text-xs"></i>
                        <span class="ml-3 text-sm font-medium text-gray-500 dark:text-gray-400 truncate max-w-xs">{{ Str::limit($news->title, 30) }}</span>
                    </div>
                </li>
            </ol>
        </nav>

        <div class="bg-white dark:bg-gray-800 shadow-2xl rounded-xl overflow-hidden transition-all duration-300 hover:shadow-3xl">
            <!-- Başlık ve Meta Bilgiler -->
            <div class="p-6 md:p-8">
                <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                    <div>
                        @if($news->menus && $news->menus->isNotEmpty())
                            <span class="inline-block px-3 py-1 text-xs font-semibold tracking-wider text-indigo-600 uppercase bg-indigo-100 rounded-full dark:bg-indigo-900 dark:text-indigo-200">
                                {{ $news->menus->first()->name }}
                            </span>
                        @endif
                        <h1 class="mt-3 text-3xl md:text-4xl font-bold text-gray-900 dark:text-white leading-tight">
                            {{ $news->title }}
                        </h1>
                    </div>
            ...
            <div class="relative">
                <div class="swiper-container rounded-lg">
                    <div class="swiper-wrapper">
                        ...
                        @empty
                            <div class="swiper-slide">
                                <div class="relative pt-[56.25%] overflow-hidden bg-gray-200 dark:bg-gray-700">
        ...
        <!-- Benzer Haberler -->
        <div class="mt-12">
            <h3 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">Benzer Haberler</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @for ($i = 0; $i < 3; $i++)
                    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300">
                        ...
                        <div class="p-4">
                            <span class="text-xs font-semibold text-indigo-600 dark:text-indigo-400 uppercase">Teknoloji</span>
                            <h4 class="mt-1 text-lg font-semibold text-gray-900 dark:text-white line-clamp-2">Yeni teknoloji trendleri ve gelecek projeksiyonları</h4>
                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400 line-clamp-2">Teknoloji dünyasındaki son gelişmeler ve önümüzdeki yıllarda bizi nelerin beklediğine dair uzman görüşleri...</p>
